import { PAYMENT_CHAIN_ID, PAYMENT_REQUIRED_CONFIRMATIONS, getPaymentTokenDefinition } from "@walty/shared/payments/config"
import type { PaymentDiscrepancy, PaymentRequestStatus, PaymentRequestView } from "@walty/shared/payments/types"
...
export function toPaymentRequestView(record: PaymentRequestRecord): PaymentRequestView {
  ...
  const discrepancy = normalized.paymentDiscrepancy as PaymentDiscrepancy | null
  ...
  return {
    ...
    payerAddress: normalized.payerAddress ?? null,
    isSplitPayment,
    totalPaidUsd,
    remainingAmountUsd,
    receivedAmountToken: normalized.receivedAmountToken ?? null,
    receivedAmountUsd: normalized.receivedAmountUsd ?? null,
